...
describe 'podman_quadlet::container::headscale' do
  ...
  on_debian_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      it 'manages no firewalld rules by default (firewalld is not installed)' do
        is_expected.not_to contain_firewalld_service('Allow HTTPS - headscale - public')
        is_expected.not_to contain_firewalld_port('Allow WireGuard - headscale - public')
      end

      context 'with firewall management enabled' do
        let(:params) { super().merge('fw' => true) }
