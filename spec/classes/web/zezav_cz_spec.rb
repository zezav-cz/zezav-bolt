  ...
  on_debian_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      it 'bootstraps over plain HTTP while no certificate exists' do
        ...
        is_expected.to contain_nginx__resource__server('www.zezav.cz').with_ssl(false)
        is_expected.to contain_nginx__resource__location('zezav.cz/').with_ssl_only(false)
      end

      it 'requests one certificate covering apex and www after nginx is up' do
      ...
      context 'with an issued certificate' do
        let(:facts) do
          os_facts.merge('letsencrypt_directory' => { 'zezav.cz' => '/etc/letsencrypt/live/zezav.cz' })
        end
