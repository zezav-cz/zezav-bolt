  ...
  on_debian_os.each do |os, os_facts|
    context "on #{os}" do
      ...
      it 'requests a certificate after nginx is up' do
        is_expected.to contain_letsencrypt__certonly('dir.zezav.cz')
          .with_domains(['dir.zezav.cz'])
          .that_requires('Class[nginx::service]')
      end

      context 'with an issued certificate' do
        let(:facts) do
          os_facts.merge('letsencrypt_directory' => { 'dir.zezav.cz' => '/etc/letsencrypt/live/dir.zezav.cz' })
        end

        it 'serves the listing over TLS without forcing an HTTPS redirect' do
          is_expected.to contain_nginx__resource__server('dir.zezav.cz')
            .with_ssl(true)
            .with_ssl_redirect(false)
            .with_ssl_cert('/etc/letsencrypt/live/dir.zezav.cz/fullchain.pem')
        end
      end
    end
  end
